...
def approve_teacher_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if teacherSalary.is_valid():
            teacher=TMODEL.Teacher.objects.get(id=pk)
            teacher.designation = teacherSalary.cleaned_data['designation']
            teacher.status=True
            teacher.save()
        else:
...
def admin_check_marks_view(request,pk):
    ...
    results= models.ExamAttempt.objects.all().filter(exam=course).filter(student=student)
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            email = sub.cleaned_data['email']   # fixed here
            name = sub.cleaned_data['name']     # fixed here
            message = sub.cleaned_data['message']  # fixed here
